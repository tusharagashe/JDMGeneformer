...
for i in pbar:
    ...
    # Update tqdm line with current trial hyperparams
    pbar.set_postfix({
    ...
    leaderboard = sorted(results, key=lambda x: x["macro_f1"], reverse=True)[:5]
    leaderboard_df = pd.DataFrame(leaderboard)
    pbar.write("\n--- Leaderboard (Top 5 by macro_f1) ---")
    pbar.write(leaderboard_df.to_string(index=False))

# Save all results to CSV
pd.DataFrame(results).to_csv(os.path.join(results_folder, "hyperparam_search_results.csv"), index=False)
